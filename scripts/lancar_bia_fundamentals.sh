...
echo "==> Lançando instância bia-fundamentals..."
...
aws ec2 run-instances \
  ...
  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=bia-fundamentals}]' \
  --iam-instance-profile Name=role-acesso-ssm \
  --user-data file://$(dirname $0)/user_data_bia_fundamentals.sh

echo "==> Instância bia-fundamentals lançada com sucesso!"
